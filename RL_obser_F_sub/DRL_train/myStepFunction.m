...
if LoggedSignals.num_steps == trainConstants.n_steps
    ...
    subFidelity_episode = data.subFidelity_episode;
    fullFidelity_episode = data.fullFidelity_episode;
    
    num_episode = num_episode + 1;

    meanReward_episode = [meanReward_episode, sum(reward_steps)/n_steps];
    subFidelity_episode = [subFidelity_episode, sum(subFidelity_steps)/n_steps];
    ...
    if ~isempty(val)
        ...
        saveas(gcf,[data_dirname,'/',DRL_dirname,'_picture/full_sub_fidelityEp_Ep',num2str(num_episode),'.png'])
        close(f)
        save([data_dirname,'/',DRL_dirname,'_picture_data/full_sub_fidelityEp_Ep',num2str(num_episode),'.mat'], ...
            'subFidelity_episode', 'fullFidelity_episode')
        ...
        xlabel('t','Interpreter','latex','FontSize',20)
        ylabel('$|\langle\psi(0)|\psi(t)\rangle|^2$','Interpreter','latex','FontSize',20)
        saveas(gcf,[data_dirname,'/',DRL_dirname,'_picture/sub_full_fidelityStep_Ep',num2str(num_episode),'.png'])
        close(f)     
        save([data_dirname,'/',DRL_dirname,'_picture_data/sub_full_fidelityStep_Ep',num2str(num_episode),'.mat'], ...
