# -*- coding: utf-8 -*-
"""
Created on Thu Apr 25 10:57:17 2024
...
def count_bits_on_odd_positions(number):
    count = 0
    position = 1
...
# parameter
numLattice = 10 #numLattice<=14
numAChain = 1#int(numLattice/2) # partial fidelity, entropy
numBChain = numLattice - numAChain # partial fidelity, entropy
...
print('basis for the A and B of sub chain')

subAChainIndex2Pattern = subChainBasisIndex2Pattern(numAChain)
subBChainIndex2Pattern = subChainBasisIndex2Pattern(numBChain)

subAChainPattern2Index = basisPattern2Index(subAChainIndex2Pattern)
subBChainPattern2Index = basisPattern2Index(subBChainIndex2Pattern)

if len(subAChainIndex2Pattern)==2**(2*numAChain) and len(subBChainIndex2Pattern)==2**(2*numBChain):
    numAChainBasis = len(subAChainIndex2Pattern)
    numBChainBasis = len(subBChainIndex2Pattern)
...
print('index map from chain basis to sub-chain basis of psi')
[upChain_row, downChain_col,subAChain_row ,subBChain_col] = \
    spinChain2subChain_row_col_table(numUpChainBasis,numDownChainBasis,numAChain,\
                                     numBChain,upChainIndex2Pattern,\
    downChainIndex2Pattern,subAChainPattern2Index,subBChainPattern2Index) 
 

print('index map from chain basis to half-chain basis of psi')
[_, _ ,halfChain_row,halfChain_col] = \
...
rundir = './numLattice_'+str(numLattice)+'_numAChain_'+str(numAChain)+'_numUp_'+str(numUp)+'_numDown_'+str(numDown)
update_dir(rundir)

# with h5py.File(rundir+'/subBChainIndex2Pattern.mat','w') as f:
    # dset = f.create_dataset('numLattice',data=numLattice)
    # dset = f.create_dataset('numHalfChain',data=numHalfChain)
    # dset = f.create_dataset('numAChain',data=numAChain)
    # dset = f.create_dataset('numBChain',data=numBChain)
    # dset = f.create_dataset('numUp',data=numUp)
    # dset = f.create_dataset('numDown',data=numDown)
    # dset = f.create_dataset('numUpChainBasis',data=numUpChainBasis)
    # dset = f.create_dataset('numDownChainBasis',data=numDownChainBasis)
    # dset = f.create_dataset('numAChainBasis',data=numAChainBasis)
    # dset = f.create_dataset('numBChainBasis',data=numBChainBasis)
    # dset = f.create_dataset('numHalfChainBasis',data=numHalfChainBasis)
    # dset = f.create_dataset('numUp_even_bit',data=numUp_even_bit)
    # dset = f.create_dataset('numDown_even_bit',data=numDown_even_bit)
    # dset = f.create_dataset('halfChainIndex2Pattern',data=halfChainIndex2Pattern)
    # dset = f.create_dataset('upChainIndex2Pattern',data=upChainIndex2Pattern)
    # dset = f.create_dataset('downChainIndex2Pattern',data=downChainIndex2Pattern)
    # dset = f.create_dataset('subAChainIndex2Pattern',data=subAChainIndex2Pattern)
    # dset = f.create_dataset('subBChainIndex2Pattern',data=subBChainIndex2Pattern)
    # dset = f.create_dataset('HUphop',data=HUphop)
    # dset = f.create_dataset('HDownhop',data=HDownhop)
    # dset = f.create_dataset('VpreUpOrDown',data=VpreUpOrDown)
    # dset = f.create_dataset('VpreUpAndDown',data=VpreUpAndDown)
    # dset = f.create_dataset('upChain_row',data=upChain_row)
    # dset = f.create_dataset('downChain_col',data=downChain_col)
    # dset = f.create_dataset('subAChain_row',data=subAChain_row)
    # dset = f.create_dataset('subBChain_col',data=subBChain_col)
    # dset = f.create_dataset('halfChain_row',data=halfChain_row)
    # dset = f.create_dataset('halfChain_col',data=halfChain_col)

# savemat(rundir+'/subBChainIndex2Pattern.mat', {'subBChainIndex2Pattern':subBChainIndex2Pattern})
